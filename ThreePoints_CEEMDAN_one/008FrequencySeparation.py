...
Pos_amp = np.abs(Pos)
# %%输出到文件
df1 = pd.DataFrame({'t':t,'theta':theta,'S': S,'S_rebuild': S_rebuild,'Sync': np.real(Sync_sig), 'Async': np.real(Async_sig), 'Rod': np.real(Rod_sig), 'Pos': np.real(Pos_sig)})
df1.to_csv('ThreePoints_CEEMDAN_one\Data\SyncAndAsyncData.csv', index=False)
df2 = pd.DataFrame({'freq_i':freq_i,'amp_i':amp_i,'sync_amp':Sync_amp[interest_freq_mask],'async_amp':Async_amp[interest_freq_mask]})
df2.to_csv('ThreePoints_CEEMDAN_one\Data\SyncAndAsyncAmpData.csv', index=False)
# %%误差绘图
# 同步误差
plt.figure(3)
